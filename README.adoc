...
The theme project build compiles assets that are agnostic to the content model so the site generator doesn't have to worry about that step.
The build then publishes the theme as a bundle.
The site generator then picks up the bundle, extracts it, and takes compilation to completion by weaving the content model into the templates to make the pages and auxiliary data files and copies the remaining assets as is.
...
When the files are built, they are assembled inside the [.path]_build_ directory.
As this directory is generated, it is safe to remove.
...
The contents of the bundle mirrors the contents of the master branch, except it doesn't contain any files other than the ones that make up the theme.
...
The HTTP port used for the preview is configured in `theme.yml` but you can override it with the command line argument `--port` like this:

  $ gulp serve --port 1337
