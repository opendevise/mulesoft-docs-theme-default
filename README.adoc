...
The {uri-ci}[CI job] takes over from there and executes the `release` build task.
...
The {uri-ci}[CI job] is already configured, so there's nothing you need to do to make a release work.
You just push a change to the master branch and a new release is made.
...
The CI job is configured in Jenkins, specifically the Jenkins instance managed by the MuleSoft doc team.
The name of the job is _docs-theme-publisher_.
This section describes in detail how that job has been configured.

The release is performed by the mule-docs-agent GitHub user, which interacts with GitHub using the GitHub API.
...
* _Description:_ Personal access token for the mule-docs-agent GitHub user (scopes: public_repo)
 ...
 ** _Project url:_ \https://github.com/mulesoft/mulesoft-docs-theme-default/
* {blank} [x] Restrict where this project can be run
 ** _Label Expression:_ lxc-fedora25
* *Source Code Management*
 ** {blank}[x] Git
 ** _Repository URL:_ \https://github.com/mulesoft/mulesoft-docs-theme-default
 ...
 ** {blank}[x] Use secret text(s) or file(s)
  *** *Secret text*
   ...
   **** _Credentials:_ some text (Personal access token for the mule-docs-agent GitHub user (scopes: public_repo))
...
Click btn:[Add webhook], enter the following URL in the Payload URL field, then click btn:[Add webhook].
 ...
 https://mulesoft-docs.ci.cloudbees.com/git/notifyCommit?url=https://github.com/mulesoft/mulesoft-docs-theme-default&branches=master
